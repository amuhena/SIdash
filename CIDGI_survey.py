...
# Custom CSS to adjust column margins and paddings
st.markdown(
    """
    <style>
    .stRadio > div {
    ...
    .stColumns {
        margin: 0px; /* Adjust margin between columns */
        padding: 0px; /* Adjust padding within columns */
    }
    .stColumn {
        padding: 0px; /* Adjust padding within individual column */
    }
    </style>
    """,
...
def home():
    ...
    with col2:
        ...
        def create_horizontal_bar_chart(labels, values):
            bar_fig = go.Figure()
            ...
            bar_fig.update_layout(
                title=dict(text=selected_graph, x=0.5, font=dict(size=14)),  # Display only the filter category as title
        ...
        def create_horizontal_clustered_bar_chart(labels, values):
            bar_fig = go.Figure()
            ...
            bar_fig.update_layout(
                barmode='group',
                title=dict(text=selected_graph, x=0.5, xanchor='center', font=dict(size=14)),  # Display only the filter category as title
                xaxis=dict(color='black'),
                yaxis=dict(color='black'),
                font=dict(color='black'),  # Set text color to black
                showlegend=False,  # Remove the legend
                margin=dict(l=10, r=10, t=25, b=30),  # Adjust margins to make space for annotations
                height=450,  # Adjust the height of the chart
                width=550  # Adjust the width of the chart
            )
            return bar_fig
        ...
        def create_pie_chart(labels, values):
            ...
            pie_fig.update_layout(
                title=dict(text='National', x=0.5, xanchor='center', font=dict(size=14)),  # Display only 'National' as title
                ...
                margin=dict(l=5, r=5, t=20, b=5),  # Adjust margins to make space for annotations
                height=500,  # Adjust the height of the chart
                width=300  # Adjust the width of the chart
            )
            return pie_fig

        # Move question text before the filter section
        ...
        # Move filter section before the bar chart
        selected_graph = st.radio("Filter", ["Ethnicity", "Sex", "Age", "Locality"], key=f"filter_radio_{selected_heading}")
        if selected_graph == "Ethnicity":
            bar_chart = create_horizontal_clustered_bar_chart(data['ethnicity'], data['ethnicity_values'])
        elif selected_graph == "Sex":
            bar_chart = create_horizontal_clustered_bar_chart(data['sex'], data['sex_values'])
        elif selected_graph == "Age":
            bar_chart = create_horizontal_clustered_bar_chart(data['age'], data['age_values'])
        elif selected_graph == "Locality":
            bar_chart = create_horizontal_clustered_bar_chart(data['locality'], data['locality_values'])
        else:
            bar_chart = None
        if bar_chart:
    ...
    with col3:
        pie_chart = create_pie_chart(data['national'], data['national_values'])
...
def full_report():
    ...
    def create_horizontal_clustered_bar_chart(labels, values, title, section):
        ...
        bar_fig.update_layout(
            ...
            font=dict(color='black'),  # Set text color to black
            showlegend=False,  # Remove the legend
            margin=dict(l=10, r=10, t=30, b=30),  # Adjust margins to make space for annotations
            height=500,  # Adjust the height of the chart
            width=800  # Adjust the width of the chart
        )
        return bar_fig
    ...
    for section in survey_data.keys():
        ...
        st.plotly_chart(create_pie_chart(data['national'], data['national_values'], 'National'))

        filter_option = st.radio("Filter by", ["Ethnicity", "Sex", "Age"], key=f"{section}_filter_radio")

        if filter_option == "Ethnicity":
            bar_chart = create_horizontal_clustered_bar_chart(data['ethnicity'], data['ethnicity_values'], 'Ethnicity', section)
        elif filter_option == "Sex":
            bar_chart = create_horizontal_clustered_bar_chart(data['sex'], data['sex_values'], 'Sex', section)
        elif filter_option == "Age":
            bar_chart = create_horizontal_clustered_bar_chart(data['age'], data['age_values'], 'Age', section)
        else:
            bar_chart = None
...
if st.session_state.page == "home":
    home()
elif st.session_state.page == "full_report":
    full_report()
